...
IIR_asm
	...
	MOV 	R4, #0		; value of n
	MOV		R5, #0		; memory address of x[n]
					
	; set values of floating point registers to zero before filtering loop
	VMOV	S1, R4	; coeffiecient register
	VMOV	S2, R4	; floating point value value of x[n]
	VMOV	S3, R4	; value of x[n-1]
	VMOV 	S4, R4	; value of x[n-2]
	VMOV	S5, R4	; value of y[n-1]
	VMOV	S6, R4	; value of y[n-2]
	VMOV	S7, R4	; value of y[n]
